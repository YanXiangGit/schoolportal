...
public class Utils {
    ...
    public  static Map<String, Object> property(String str,String [] zd ) {  //陈斌添加解析字符串为map
        Map<String, Object> pptyMap = new HashMap<String, Object>();
        String [] property = str.split("\\$");
        for(int i=0;i<zd.length;i++){
            for(int j=0;j<property.length;j++){

                if(property[j].indexOf(zd[i]+"：")!=-1){
                    String [] value = property[j].split("\\：");
                    pptyMap.put(zd[i], value[1].trim());
                }
            }
        }
        return  pptyMap;
    }
    public  static String  getUsername( List<Map<String, Object>> userlist ,String xyid) {  //陈斌添加 获取从账号
        String username="";
        for(int i=0;i<userlist.size();i++){
            if(userlist.get(i).get("XTID").equals(xyid)){
                username=userlist.get(i).get("USER")+"";
            }
        }
        return  username;
    }
    public  static  List<Map<String, Object>>  sortingList(List<Map<String, Object>> list) {  //陈斌添加对list<map> 排序
        Collections.sort(list, new Comparator<Map<String, Object>>() {
            public int compare(Map<String, Object> o1, Map<String, Object> o2) {
                Integer name1 = Integer.valueOf(o1.get("XH").toString()) ;
                Integer name2 = Integer.valueOf(o2.get("XH").toString()) ;
